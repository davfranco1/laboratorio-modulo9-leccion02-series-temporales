...
class SARIMAModel:
        ...
        for p, q, seasonal_order in tqdm(parametros):
            try:
                ...
                if self.best_model is None or error < self.best_model["RMSE"]:
                    ...
                    print(f"Los parámetros del mejor modelo son {self.best_params}, con un RMSE de {error}")
    ...
    def predecir(self, dataframe, order, seasonal_order, steps):
        # Entrenamiento con todos los datos
        modelo_sarima_full = SARIMAX(
            dataframe,
            order=order,  # p, d, q
            seasonal_order=seasonal_order  # P, D, Q, m
        ).fit(disp=False)

        prediccion = pd.DataFrame(modelo_sarima_full.forecast(steps=steps))
        return prediccion
    
    def plot_prediccion(self, y_test, dataframe_prediccion, columna_predicha):
        plt.figure(figsize=(15,5))
        sns.lineplot(x = y_test.index, y = columna_predicha, data = y_test, label="Reales")
        sns.lineplot(x = dataframe_prediccion.index, y = "predicted_mean", data = dataframe_prediccion, label="Predichos" , linestyle= "--")
        plt.show()
